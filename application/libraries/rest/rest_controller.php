...
class RestController extends \CI_Controller
{
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        // }
        if (!method_exists($this, $controller_method)) {
          $controller_method = 'page_not_found';
        }
...
# =======================================================================================================================================
# =================                               Custom RestController functions                                       =================
# =======================================================================================================================================

    public function special_get($keys) {
        if(is_array($keys)) {
            $data = array();
            foreach ($keys as $key) {
                $key = trim($key);
                $get = $this->get($key);
                if($get == null) {
                    $data = array_merge($data, ["$key" => null]);
                } else {
                    if(is_string($get)) {
                        $get = htmlspecialchars($get);
                        $data = array_merge($data, ["$key" => trim($get)]);
                    } else if(is_array($get)) {
                        $array = array();
                        foreach ($get as $p => $value) {
            ...
            return new RequestData(['method' => 'GET', 'data' => (object) $data]);
            ...
            return $this->special_get($fix);
        }

        return new RequestData(['method' => null, 'data' => null]);
    }

    public function special_post($keys) {
        if(is_array($keys)) {
            $data = array();
            foreach ($keys as $key) {
                $key = trim($key);
                ...
                if($post == null) {
                    ...
                    if(is_string($post)) {
                        ...
                        $data = array_merge($data, ["$key" => trim($post)]);
            ...
            return new RequestData(['method' => 'POST', 'data' => (object) $data]);
        } else if(is_string($keys)) {
            $keys = explode(',', $keys);
            $fix = array();
            ...
            return $this->special_post($fix);
        }

        return new RequestData(['method' => null, 'data' => null]);
    }

    public function file_post($key) {
      if($key == null || (is_array($key) && count($key) < 1)) return null;

      $result = null;
      if(is_array($key)) {
        for($i = 0; $i < count($key); $i++) {
          if(array_key_exists($key[$i], $_FILES)) {
        ...
        if(array_key_exists("$key", $_FILES)) $result = $_FILES[$key];
      }

      return $result;
    }

    public function multipart_post($key) {
      $value;
      if(is_array($key)) {
        $value = array();
        for($i = 0; $i < count($key); $i++) {
          $data = htmlspecialchars($this->input->post("{$key[$i]}"));
          $data = $data !== null && strlen($data) > 0 ? trim($data) : null;
          $value = array_merge($value, array($key[$i] => $data));
        }
      } else {
    ...
    public function special_put($keys) {
        if(is_array($keys)) {
            $data = array();
            foreach ($keys as $key) {
                $key = trim($key);
                ...
                if($put == null) {
                    ...
                    if(is_string($put)) {
                        $put = htmlspecialchars($put);
                        $data = array_merge($data, ["$key" => trim($put)]);
                    } else if(is_array($put)) {
                        $array = array();
                        foreach ($put as $p => $value) {
                            $value = htmlspecialchars($value);
                            $array = array_merge($array, ["$p" => trim($value)]);
                        }
            ...
            return $this->special_put($fix);
        }

        return new RequestData(['method' => null, 'data' => null]);
    }

    public function special_delete($key) {
      $value;
      if(is_array($key)) {
        $value = array();
        for($i = 0; $i < count($key); $i++) {
          $data = htmlspecialchars($this->delete($key[$i]));
          $data = $data !== null && strlen(trim($data)) > 0 ? trim($data) : null;
          $value = array_merge($value, array($key[$i] => $data));
        }
      } else {
        $value = htmlspecialchars($this->delete($key));
        $value = $value !== null && strlen(trim($value)) > 0 ? trim($value) : null;
      }

      return $value;
    }

    public function special_patch($keys) {
        if(is_array($keys)) {
            $data = array();
            foreach ($keys as $key) {
                $key = trim($key);
                $patch = $this->patch($key);
                if($patch == null) {
                    $data = array_merge($data, ["$key" => null]);
                } else {
                    if(is_string($patch)) {
                        $patch = htmlspecialchars($patch);
                        $data = array_merge($data, ["$key" => trim($patch)]);
            ...
            return new RequestData(['method' => 'PATCH', 'data' => (object) $data]);
        } else if(is_string($keys)) {
            $keys = explode(',', $keys);
            $fix = array();
            ...
            return $this->special_patch($fix);
        }

        return new RequestData(['method' => null, 'data' => null]);
    }

    public function special_options($key) {
      $value;
      if(is_array($key)) {
        $value = array();
        for($i = 0; $i < count($key); $i++) {
          $data = htmlspecialchars($this->options($key[$i]));
          $data = $data !== null && strlen(trim($data)) > 0 ? trim($data) : null;
          $value = array_merge($value, array($key[$i] => $data));
        }
      } else {
        $value = htmlspecialchars($this->options($key));
        $value = $value !== null && strlen(trim($value)) > 0 ? trim($value) : null;
      }
    ...
    public function special_head($key) {
      $value;
      if(is_array($key)) {
        $value = array();
        for($i = 0; $i < count($key); $i++) {
          $data = htmlspecialchars($this->head($key[$i]));
          $data = $data !== null && strlen(trim($data)) > 0 ? trim($data) : null;
          $value = array_merge($value, array($key[$i] => $data));
        }
      } else {
        $value = htmlspecialchars($this->head($key));
        $value = $value !== null && strlen(trim($value)) > 0 ? trim($value) : null;
      }
    ...
    public function response_ok($data = []) {
      $code = (int) RestController::HTTP_OK;

      $data['success'] = true;
      $data['code'] = $code;

      $this->response(new ResponseData($data), $code);
      die;
    }

    public function bad_response($data = []) {
    ...
    public function uri_segment($segment) {
      ...
      $uri = urldecode($uri);
      $uri = explode('/', $uri);
      if(count($uri) < $segment) {
        // $segment = $segment + 1;
        $segment = $segment - count($uri);
        for($a = 0; $a < $segment; $a++) {
          array_push($uri, null);
        }
      }

      $value = $uri[$i] !== null && strlen($uri[$i]) > 0 ? trim($uri[$i]) : null;
    ...
    public function is_auth() {
        ...
        if(!isset($authHeader))
            $this->bad_response(['message' => 'Authentication Required for this request!', 'type' => 'authentication_failed']);

        $authHeader = str_replace('Bearer', '', $authHeader);
        $authHeader = trim($authHeader);

        $decodedToken = decode_jwt($authHeader);

        if(is_bool($decodedToken) && $decodedToken == false)
            $this->bad_response(['message' => 'Authentication Required for this request!', 'type' => 'authentication_failed']);

        $ci->load->model('User_model');
        $userModel = array('id' => $decodedToken->uid);
        $userModel = to_user_model($userModel);
        $userModel->get_data('id');

        if($userModel->email == null)
            $this->bad_response(['message' => 'Authentication Required for this request!', 'type' => 'authentication_failed']);

        $userModel->auth_token = encode_jwt($userModel->id);
        unset($userModel->password);
        $this->auth_data = $userModel;
    }
    ...
    public function is_admin() {
        if(is_bool($this->auth_data) && $this->auth_data == false) 
            $this->is_auth();

        if($this->auth_data->role !== 'admin')
            $this->bad_response(['message' => 'Authentication Required for this request!, access restricted', 'type' => 'authentication_failed']);
    }
...
class ResponseData {
    ...
    function __construct($data = []) {
        ...
        if(property_exists($data, 'success') && is_bool($data->success)) {
            $this->success = (bool) $data->success;
        } else {
            unset($this->success);
        }

        if(property_exists($data, 'code') && is_int($data->code)) {
        ...
        if(property_exists($data, 'next_token')) {
            $this->next_token = (String) $data->next_token;
        } else {
...
require_once APPPATH . "libraries/rest/format.php";
